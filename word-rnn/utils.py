...
class TextLoader():
	...
	words = [word for word in word_counts if word_counts[word] >= 10]
	...
	# print(vocabulary)
	# os.exit(0)
	assert(len(vocabulary) == len(vocabulary_inv))
        return [vocabulary, vocabulary_inv]

    def preprocess(self, input_file, vocab_file, tensor_file, encoding):
        with open(input_file, "r") as f:#, encoding=encoding) as f:
            data = f.read()
	...
	for x in x_text:
	    if x in self.vocab:
		l.append(self.vocab[x])
	self.tensor = np.array(l)
